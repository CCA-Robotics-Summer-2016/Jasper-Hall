/*
 Jasper's code for the ultrasonic distance-sensing obstacle-avoiding robot, 
 code taken from the class github and modified to meet my needs
 */
...
void loop() {
  ...
  if (distance >= 200 || distance <= 0) {
    ...
    if (distance < minimumDistance) {
      //pivot right, take measurement
      //pivot left, take measurement
      ...
      pivotRight (1500); //return to forward position
      stopRobot (200); 
      ...
      /* tried to add the following code but kept getting error message "expected '}' before 'else'"
        else (leftDistance <=25
               || rightDistance <=25)
       {        
        goReverse (500)
       }       */
...
//The section that follows is used to define the functions used in this code loop
  ...
  void pivotLeft (int timeToMove) {
    Serial.print ("pivot right for ");
    Serial.println (timeToMove);
  ...
  void pivotRight (int timeToMove) {
    Serial.print ("pivot left for ");
    Serial.println (timeToMove);
    digitalWrite (in1, HIGH);
    digitalWrite (in2, LOW);
